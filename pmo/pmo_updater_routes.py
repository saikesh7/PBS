    ...
    try:
        ...
        try:
            ...
            if category_ids:
                ...
                # ✅ FIXED: Only show requests created by this PMO updater
                ...
                # ✅ FIXED: Only show records actually created by this PMO updater
                old_history_cursor = mongo.db.points_request.find({
                    "status": {"$in": ["Pending", "Approved", "Rejected"]},
                    "created_by_pmo_id": ObjectId(user['_id'])  # Only records created by this updater
                }).sort("request_date", -1)
            else:
                history_cursor = []
                old_history_cursor = []
            ...
            for req in combined_cursor:
                ...
                if emp:
                        ...
                        # Get submitted by (who created the request)
                        submitted_by_id = req.get("created_by_pmo_id")
                        submitted_by_name = "N/A"
                        if submitted_by_id:
                            submitted_by_user = mongo.db.users.find_one({"_id": submitted_by_id}, {"name": 1})
                            if submitted_by_user:
                                submitted_by_name = submitted_by_user.get("name", "N/A")
                        
                        # Get submitted to (validator who will review the request)
                        validator_id = req.get("assigned_validator_id") or req.get("pending_validator_id") or req.get("processed_by")
                        submitted_to_name = "N/A"
                        if validator_id:
                            validator_user = mongo.db.users.find_one({"_id": validator_id}, {"name": 1})
                            if validator_user:
                                submitted_to_name = validator_user.get("name", "N/A")
                        
                        # Calculate quarter from event_date (fiscal year: April-March)
                        quarter_display = "N/A"
                        if event_date and isinstance(event_date, datetime):
                            month = event_date.month
                            year = event_date.year
                            if 4 <= month <= 6:
                                quarter_display = f"Q1-{year}"
                            elif 7 <= month <= 9:
                                quarter_display = f"Q2-{year}"
                            elif 10 <= month <= 12:
                                quarter_display = f"Q3-{year}"
                            else:  # 1-3 (Jan-Mar) - belongs to previous fiscal year
                                quarter_display = f"Q4-{year - 1}"
                        ...
                        history.append({
                            ...
                            'submitted_to': submitted_to_name,
            ...
            if category_ids:
                points_cursor = mongo.db.points.find({
                    "$or": [
                        ...
                        {"created_by_pmo_id": ObjectId(user['_id'])}  # Another old field
                    ],
                    "category_id": {"$in": category_ids}
                }).sort("created_at", -1)
            else:
            ...
            for point in points_cursor:
                    ...
                    if emp:
                        ...
                        # Get submitted by (awarded_by is the PMO updater who created it)
                        submitted_by_id = point.get("awarded_by")
                        submitted_by_name = "N/A"
                        if submitted_by_id:
                            submitted_by_user = mongo.db.users.find_one({"_id": submitted_by_id}, {"name": 1})
                            if submitted_by_user:
                        ...
                        # Get submitted to (validator) - for old approved records, show "Approved"
                        submitted_to_name = "Approved"
            ...
            def sort_key(x):
                ...
                date_val = x['request_date'] if x['request_date'] else datetime.min
                return (status_priority.get(x['status'], 4), date_val)
            
            # Sort with reverse=True to get most recent dates first
            ...
            try:
                ...
                if department and employee_department:
                    ...
                    if department.strip().lower() != employee_department.strip().lower():
                        raise ValueError(f"Wrong department '{department}'. Employee '{employee_name}' ({employee_id}) belongs to '{employee_department}'")
                ...
                # Get points based on grade (matching TA logic - no fallback to base)
        ...
        for row in validated_rows:
            try:
                ...
                if employee and validator:
                    ...
                    # Ensure created_by_pmo_id is set for proper routing
...
def validate_utilization_upload():
    ...
    try:
        ...
        # ✅ Track employee-month combinations to detect duplicates within the CSV
        employee_month_tracker = {}  # Key: "employee_id|YYYY-MM", Value: row_number
        ...
        for row_num, row in enumerate(csv_reader, start=2):
            try:
                ...
                # ✅ Check for duplicate employee-month combination within the CSV file
                month_key = f"{employee_id}|{event_date.strftime('%Y-%m')}"
                if month_key in employee_month_tracker:
                    previous_row = employee_month_tracker[month_key]
                    raise ValueError(f"⚠️ Duplicate utilization for {event_date.strftime('%B %Y')}. This employee already has a utilization entry in row {previous_row} for the same month. Only one utilization per employee per month is allowed.")
                
                # Track this employee-month combination
                employee_month_tracker[month_key] = row_num
                
                # ✅ MODIFIED: Check for existing utilization record in the same month
                # Now allows multiple uploads per month - system will analyze and handle per-month data
                start_of_month = event_date.replace(day=1, hour=0, minute=0, second=0, microsecond=0)
                next_month = (start_of_month.replace(month=start_of_month.month % 12 + 1) if start_of_month.month < 12 
                             else start_of_month.replace(year=start_of_month.year + 1, month=1))
                
                # Check for existing APPROVED record in the same month (prevent duplicate approved records)
                existing_approved = mongo.db.points_request.find_one({
                    "user_id": ObjectId(employee["_id"]),
                    "category_id": ObjectId(category["_id"]),
                    "event_date": {"$gte": start_of_month, "$lt": next_month},
                    "status": "Approved"
                ...
                if existing_approved:
                    raise ValueError(f"⚠️ Utilization already approved for {event_date.strftime('%B %Y')}. Cannot upload duplicate approved records.")
...
@pmo_bp.route('/updater/delete-record', methods=['POST'])
...
@pmo_bp.route('/updater/modify-record', methods=['POST'])
