...
def get_pmo_categories_for_validator():
    """Get ALL active PMO categories for validator display (includes employee_raised)"""
    mongo = get_mongo()
    # Include ALL PMO categories including employee_raised for validators
    categories = list(mongo.db.hr_categories.find({
        'category_department': {'$regex': '^pmo', '$options': 'i'},
        'category_status': 'active'
    }).sort('name', 1))
...
def get_all_pmo_categories():
    """
    Get ALL PMO categories (both active and inactive) from hr_categories collection
    ...
    """
    mongo = get_mongo()
    # Use regex for case-insensitive matching (no status filter)
    categories = list(mongo.db.hr_categories.find({
        'category_department': {'$regex': '^pmo', '$options': 'i'}
    }).sort('name', 1))
    return categories
...
def get_all_pmo_category_ids():
    """Get list of ALL PMO category ObjectIds (active + inactive) - for displaying existing requests"""
    categories = get_all_pmo_categories()
    return [cat["_id"] for cat in categories]


def get_pmo_category_ids():
    """Get list of ACTIVE PMO category ObjectIds - for new requests"""
    categories = get_pmo_categories()
    return [cat["_id"] for cat in categories]
