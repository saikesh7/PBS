...
def get_quarters_in_year(fiscal_year=None, include_future=False):
    """Get quarters in a fiscal year
    
    Args:
        fiscal_year: The fiscal year to get quarters for (defaults to current fiscal year)
        include_future: If False (default), only include quarters up to current quarter
                       If True, include all 4 quarters
    
    For dashboard display: Shows only Q1-Q4 of the specified fiscal year (2025).
    
    Quarter naming format: Q1-2025 (for fiscal year 2025-26)
    """
    now = datetime.now()
    ...
    # If showing current fiscal year and not including future quarters
    if fiscal_year == current_fiscal_year and not include_future:
        # Add all 4 quarters from current fiscal year (Q1-Q4 of 2025 only)
        for q in range(1, 5):
        ...
        # Include all 4 quarters for the specified fiscal year
        for q in range(1, 5):
            start_date, end_date = get_quarter_date_range(q, fiscal_year)
            quarters.append({
                'quarter': q,
