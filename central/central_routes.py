...
def dashboard():
    ...
    try:
        ...
        # Calculate valid date range for the current fiscal year (April 1 to March 31)
        fiscal_year_start = datetime(current_year, 4, 1)
        fiscal_year_end = datetime(current_year + 1, 3, 31)
        valid_start_date = fiscal_year_start.strftime('%d-%m-%Y')
        valid_end_date = fiscal_year_end.strftime('%d-%m-%Y')
        valid_start_date_input = fiscal_year_start.strftime('%Y-%m-%d')
        valid_end_date_input = fiscal_year_end.strftime('%Y-%m-%d')
        
        # Create fiscal year label (e.g., "2025-26")
        fiscal_year_label = f"{current_year}-{str(current_year + 1)[-2:]}"
        ...
        return render_template(
            ...
            valid_start_date_input=valid_start_date_input,
            valid_end_date_input=valid_end_date_input
        )
...
def export_data():
    ...
    try:
        ...
        # ✅ FIXED: When filtering by date, use the year from the filter dates to generate quarters dynamically
        if use_date_filter:
            # Extract year from filter dates and generate quarters for that year
            filter_year = qtr_start.year
            all_quarters_list = get_quarters_in_year(filter_year, include_future=True)
        else:
        ...
        # ✅ FIXED: Calculate utilization for the ENTIRE FISCAL YEAR based on filter or current year
        # When filtering, use the filtered year; otherwise use current year
        if use_date_filter:
            filter_year = qtr_start.year
            fiscal_year_start = datetime(filter_year, 4, 1)  # April 1st of filtered year
            fiscal_year_end = datetime(filter_year + 1, 3, 31, 23, 59, 59, 999999)  # March 31st next year
        else:
        ...
        # ✅ FIXED: Calculate valid date range based on filter or current fiscal year
        # When filtering, use the filtered year for valid date range display
        if use_date_filter:
            display_year = qtr_start.year
        else:
            display_year = current_year
        
        fiscal_year_start = datetime(display_year, 4, 1)
        fiscal_year_end = datetime(display_year + 1, 3, 31)
        valid_start_date = fiscal_year_start.strftime('%d-%m-%Y')
        valid_end_date = fiscal_year_end.strftime('%d-%m-%Y')
        valid_start_date_input = fiscal_year_start.strftime('%Y-%m-%d')
        valid_end_date_input = fiscal_year_end.strftime('%Y-%m-%d')
        ...
        # Create fiscal year label (e.g., "2025-26")
        fiscal_year_label = f"{display_year}-{str(display_year + 1)[-2:]}"
