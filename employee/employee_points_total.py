...
def get_fiscal_period_date_range(fiscal_quarter, fiscal_year_start_calendar_year):
    ...
    Same logic as employee_leaderboard.py
    """
    if fiscal_quarter == 1:
...
def filter_points():
    """API endpoint to filter points based on criteria - Using same logic as leaderboard"""
    user_id = session.get('user_id')
    if not user_id:
    ...
    try:
        ...
        # ==========================================
        # BUILD DATE RANGE FILTER (Same as leaderboard)
        # ==========================================
        date_range = None
        ...
        for req in approved_requests:
            try:
                ...
                # ==========================================
                # Apply date range filter (Same as leaderboard)
                # ==========================================
                if date_range:
