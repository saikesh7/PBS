...
def send_validator_notification(request_doc, employee, validator, category):
    ...
    try:
        ...
        if isinstance(request_date, datetime):
            formatted_date = request_date.strftime('%B %d, %Y')
        else:
        ...
        # Send email with reduced timeout (now runs in background thread, so timeout is less critical)
...
def submit_request():
    ...
    try:
        ...
        # ✅ RULE 1: Server-side validation - Check if employee is eligible for Presales category
        if category_department == 'presales':
            employee_grade = user.get('grade', '').strip().upper()
            ineligible_grades = ['A1', 'B1', 'B2']
            
            if employee_grade in ineligible_grades:
                flash(f'You are not eligible for Presales category. Your grade ({employee_grade}) cannot raise Presales requests.', 'danger')
                return redirect(url_for('employee_raise_request.raise_request'))
        ...
        if is_pm_category:
            ...
            # ✅ RULE 2: Server-side validation - Check if validator has eligible grade
            validator_grade = validator.get('grade', '').strip().upper()
            ineligible_validator_grades = ['A1', 'B1', 'B2']
            
            if validator_grade in ineligible_validator_grades:
                flash(f'Selected validator ({validator.get("name")}) with grade {validator_grade} is not eligible to validate requests. Please select another validator.', 'danger')
                return redirect(url_for('employee_raise_request.raise_request'))
        ...
        if grade_points and isinstance(grade_points, dict) and user_grade in grade_points:
            ...
            points = grade_points.get(user_grade, 0)
        else:
            ...
            if isinstance(points_per_unit, dict):
                # Try to get points for user's grade, fallback to 'base' if not found
                points = points_per_unit.get(user_grade, points_per_unit.get('base', 0))
            else:
                # If points_per_unit is not a dict, try to convert it directly
                points = int(points_per_unit) if points_per_unit else 0
        
        # ✅ SAFETY CHECK: Ensure points is a valid integer
        try:
        ...
        # ✅ EVENTLET COMPATIBLE: Call directly (eventlet handles async internally)
        try:
            ...
            if validator_user:
                # Call directly - eventlet will handle this efficiently
                publish_request_raised(request_doc, user, validator_user, category)
        except Exception as e:
            import traceback
            print(f"❌ Error publishing real-time event: {e}")
            traceback.print_exc()
        
        # Send email notification to assigned validator (single email, no duplicates)
        # ✅ EVENTLET COMPATIBLE: Use eventlet.spawn for background tasks
        try:
            import eventlet
            validator_user = mongo.db.users.find_one({"_id": ObjectId(validator_id)})
            if validator_user:
                # Use eventlet.spawn for non-blocking email sending
                eventlet.spawn(send_validator_notification, request_doc, user, validator_user, category)
        except Exception as e:
            pass
...
def get_validators_by_category(category_id):
    ...
    try:
        ...
        # ✅ RULE 1: Check if employee is eligible for Presales category
        user = mongo.db.users.find_one({'_id': ObjectId(user_id)})
        if user and category_department == 'presales':
            employee_grade = user.get('grade', '').strip().upper()
            ineligible_grades = ['A1', 'B1', 'B2']
            
            if employee_grade in ineligible_grades:
                return jsonify({
                    'error': 'You are not eligible for Presales category.',
                    'success': False,
                    'is_presales_ineligible': True,
        ...
        # ✅ RULE 2: Exclude validators with grades A1, B1, B2
        ineligible_validator_grades = ['A1', 'B1', 'B2']
        ...
        validators = list(mongo.db.users.find({
            ...
            'grade': {'$nin': ineligible_validator_grades}  # Exclude A1, B1, B2 validators
        }, {
            '_id': 1,
            'name': 1,
            'employee_id': 1,
...
def get_category_details(category_id):
    ...
    try:
        ...
        # ✅ FIXED: Handle both grade_points (Presales, PMArch, Marketing) and points_per_unit (PM, TA, etc.)
        grade_points = category.get('grade_points', {})
        points_per_unit = 0  # Default to 0
        ...
        if grade_points and isinstance(grade_points, dict) and user_grade in grade_points:
            ...
            if isinstance(points_per_unit_config, dict):
                ...
                # If points_per_unit_config is not a dict, use it directly
                points_per_unit = points_per_unit_config
        
        # ✅ SAFETY CHECK: Ensure points_per_unit is a valid integer
        try:
            points_per_unit = int(points_per_unit) if points_per_unit is not None else 0
        except (ValueError, TypeError):
            points_per_unit = 0
