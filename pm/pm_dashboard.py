...
def dashboard():
    ...
    try:
        ...
        # ✅ Get PM categories from hr_categories (active only for reference)
        pm_categories = list(mongo.db.hr_categories.find({
            "category_department": "pm",
            "category_status": "active"
        }))
        
        # Get ALL PM categories (active + inactive) for displaying existing requests
        all_pm_categories = list(mongo.db.hr_categories.find({
            "category_department": "pm"
        }))
        
        if not pm_categories and not all_pm_categories:
            flash('PM categories not found in hr_categories', 'warning')
            pm_categories = []
            all_pm_categories = []
        
        pm_category_ids = [cat["_id"] for cat in pm_categories]
        all_pm_category_ids = [cat["_id"] for cat in all_pm_categories]
        
        # Get dashboard counts - ONLY for PM category requests assigned to this PM (includes inactive)
        pending_count = mongo.db.points_request.count_documents({
            "status": "Pending",
            "category_id": {"$in": all_pm_category_ids},  # ✅ All PM categories (active + inactive)
            "$or": [
        ...
        # ✅ Build query conditions for processed_department filter
        # 1. New records with processed_department = "pm"
        # 2. Old records without processed_department but with pm category_id
        pm_dept_conditions = [
            {"processed_department": "pm"}
        ]
        if all_pm_category_ids:
            pm_dept_conditions.append({
                "processed_department": {"$exists": False},
                "category_id": {"$in": all_pm_category_ids}
            })
        
        # ✅ Count ALL processed records (not filtered by quarter) to match processed requests page
        processed_this_quarter = mongo.db.points_request.count_documents({
            ...
            "$and": [
                ...
                {"$or": pm_dept_conditions}
            ]
        })
        
        # ✅ Count ALL approved records (not filtered by quarter)
        approved_query = {
            "status": "Approved",
            "$and": [
                {
        ...
        approved_this_quarter = mongo.db.points_request.count_documents(approved_query)
        ...
        # Get recent pending requests (top 5) - ONLY PM category requests assigned to this PM (includes inactive)
        recent_pending = []
        ...
        # Get rejected requests - ONLY PM department processed by this PM
        ...
        # Get recent processed requests - ONLY PM department processed by this PM
        # Include records where:
        # 1. Originally processed by this user (processed_by matches)
        # 2. OR assigned to this user but modified by HR (assigned_validator_id matches + hr_modified flag)
        recent_processed = []
