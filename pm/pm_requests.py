...
def get_pm_categories():
    """
    Get all ACTIVE PM categories from hr_categories collection
    Used for creating new requests - only shows active categories
    """
...
def get_pm_category_ids():
    """Get list of ACTIVE PM category ObjectIds - for new requests"""
    categories = get_pm_categories()
    return [cat["_id"] for cat in categories]


def get_all_pm_categories():
    """
    Get ALL PM categories (both active and inactive) from hr_categories collection
...
def get_all_pm_category_ids():
    """Get list of ALL PM category ObjectIds (active + inactive) - for displaying existing requests"""
    categories = get_all_pm_categories()
    return [cat["_id"] for cat in categories]
...
def pending_requests():
    ...
    try:
        ...
        all_pm_category_ids = get_all_pm_category_ids()
        
        # Get ONLY requests assigned to this PM user (includes inactive categories)
...
def processed_requests():
    ...
    try:
        ...
        if not all_pm_category_ids:
            # No PM categories found, return empty list
            all_records = []
        else:
            # ✅ Fetch processed history - ONLY PM department requests
            # Filter by processed_department to show only requests that were processed as PM
            # This ensures that if a category is moved to another department, old records stay with PM
            all_records = []
            ...
            or_conditions = [
                {"processed_department": "pm"}  # New records with processed_department
            ]
...
def api_pending_count():
    ...
    try:
        # Get ALL PM categories (active + inactive) for counting existing requests
        all_pm_category_ids = get_all_pm_category_ids()
