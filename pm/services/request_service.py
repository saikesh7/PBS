"""
Request Service
Handles business logic for PM request processing
"""
...
class RequestService:
    """Service for handling PM request operations"""
    ...
    def approve_request(request_id, user_id, notes):
        ...
        try:
            ...
            processed_department = category.get("category_department", "pm") if category else "pm"
        ...
        try:
            ...
            if file_meta:
                ...
                attachment_info.update({
                    ...
                    "size": size,
                    "download_url": f"/pm/get_attachment/{str(request_data['_id'])}",
    ...
    def get_pending_requests(user_id, category_ids):
        """Get all pending requests for a user - includes old data from categories collection"""
        try:
            # Remove category filter to show ALL pending requests including old data
            cursor = mongo.db.points_request.find({
                "status": "Pending",
    ...
    def get_processed_requests(user_id, category_ids, limit=200):
        """Get processed requests history - includes old data from categories collection"""
        try:
            # Remove category filter to show ALL processed requests including old data
            cursor = mongo.db.points_request.find({
