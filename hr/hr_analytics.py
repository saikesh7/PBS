...
def get_quarterly_performance_data_fixed(filter_start_date=None, filter_end_date=None, location_filter=None):
    ...
    if location_filter:
        user_query = {"role": {"$in": ["Employee", "Manager", "HR"]}}
        ...
        # If no eligible users found, return empty quarters for current fiscal year
        if not eligible_user_ids:
            yr = fiscal_year
            for q in [1, 2, 3, 4]:
                period_name = f"Q{q} {yr}-{yr+1}"
    ...
    if filter_start_date and filter_end_date:
        ...
        # Show all 4 quarters of the current fiscal year (Q1, Q2, Q3, Q4)
        quarters_to_show = []
        yr = fiscal_year  # Current fiscal year (e.g., 2025 for Apr 2025 - Mar 2026)
        
        # Q1: Apr-Jun
        q_start = datetime(yr, 4, 1)
        q_end = datetime(yr, 6, 30, 23, 59, 59, 999999)
        quarters_to_show.append((1, yr, q_start, q_end))
        
        # Q2: Jul-Sep
        q_start = datetime(yr, 7, 1)
        q_end = datetime(yr, 9, 30, 23, 59, 59, 999999)
        quarters_to_show.append((2, yr, q_start, q_end))
        
        # Q3: Oct-Dec
        q_start = datetime(yr, 10, 1)
        q_end = datetime(yr, 12, 31, 23, 59, 59, 999999)
        quarters_to_show.append((3, yr, q_start, q_end))
        
        # Q4: Jan-Mar (next calendar year)
        q_start = datetime(yr + 1, 1, 1)
        q_end = datetime(yr + 1, 3, 31, 23, 59, 59, 999999)
        quarters_to_show.append((4, yr, q_start, q_end))
        
        # Reverse to show current quarter first (Q3, Q2, Q1, Q4 order based on current quarter)
        # Sort by quarter number descending, but put future quarters at the end
        current_date = datetime.utcnow()
        quarters_to_show.sort(key=lambda x: (x[2] > current_date, -x[0] if x[2] <= current_date else x[0]))
    ...
    for category in categories:
        ...
        for user_id in user_ids:
            ...
            if user and "grade" in user:
                ...
                # ✅ FIXED: Include ALL roles (Employee, Manager, HR) in participation count
                if user.get("role") in ["Employee", "Manager", "HR"]:
                    grade = user["grade"]
...
def get_activity_participation_fixed(start_date, end_date, location_filter=None):
    ...
    # ✅ FIXED: Include ALL employees, managers, and HR users
    user_query = {
    ...
    for category in categories:
        ...
        # ✅ FIXED: Only add date filter if BOTH start and end dates are provided
        if start_date and end_date:
            query["$or"] = [
                {"event_date": {"$gte": start_date, "$lte": end_date}},
                {"award_date": {"$gte": start_date, "$lte": end_date}},
        ...
        for entry in points_data:
            # ✅ FIXED: Only validate dates if filter is applied
            if start_date and end_date:
                event_date = extract_event_date(entry)
                entry_with_dates = {
                    'event_date': event_date,
...
def get_utilization_participation_data(start_date, end_date, location_filter=None):
    ...
    # Count eligible participants (Employees, Managers with manager_id, and HR users)
    eligible_participants = 0
    ...
    for user_id in unique_user_ids:
        try:
            ...
            if user:
                ...
                # ✅ FIXED: Include HR role in analytics
                if user.get("role") in ["Employee", "HR"] or \
...
def hr_dashboard():
    ...
    # Get total employees count (including HR users)
    ...
    # ✅ FIXED: Calculate average participation rate from ALL activities (including 0%)
    activity_participation = get_activity_participation_fixed(None, None, None)
    avg_participation_rate = 0
    if activity_participation and len(activity_participation) > 0:
        total_rate_sum = 0
        for activity in activity_participation:
            rate = activity.get("rate")
            if rate is not None:
                total_rate_sum += float(rate)
        # Calculate average from ALL activities (use 2 decimal places)
        avg_participation_rate = round(total_rate_sum / len(activity_participation), 2)
    ...
    if activity_participation and len(activity_participation) > 0:
        ...
        for activity in activity_participation:
            ...
            if rate is not None:
                total_rate_sum += float(rate)
                activity_count += 1
        # Calculate average from ALL activities
        if activity_count > 0:
            # Use 2 decimal places to show small percentages (e.g., 0.05%)
            avg_participation_rate_value = round(total_rate_sum / activity_count, 2)
    ...
    if activity_participation and len(activity_participation) > 0:
        ...
        for activity in activity_participation:
            ...
            if rate is not None:
                total_rate_sum += float(rate)
        # Calculate average from ALL activities (use 2 decimal places)
        avg_participation_rate_value = round(total_rate_sum / len(activity_participation), 2)
