    ...
    try:
        ...
        try:
            ...
            if category_ids:
                ...
                # ✅ FIXED: Only show requests created by this HR updater
                ...
                # ✅ FIXED: Only show records actually created by this HR updater
                old_history_cursor = mongo.db.points_request.find({
                    "status": {"$in": ["Pending", "Approved", "Rejected"]},
                    "created_by_hr_id": ObjectId(user['_id'])  # Only records created by this updater
                }).sort("request_date", -1)
            else:
                history_cursor = []
                old_history_cursor = []
            ...
            for req in combined_cursor:
                ...
                if emp and cat:
                    ...
                    history.append({
                        ...
                        'category_name': cat.get("name", "No Category"),
            ...
            if category_ids:
                ...
                for point in points_cursor:
                    ...
                    if emp and cat:
                        ...
                        # For old approved records, show "Approved"
...
def handle_single_assignment(user):
    ...
    if validator:
        ...
        if 'category_department' in category:
            ...
            category['category_department'] = 'hr_up'
        
        # Ensure created_by_hr_id is set for proper routing
        request_data['created_by_hr_id'] = ObjectId(user['_id'])
        ...
        for validator_id, data in validators_notified.items():
            ...
            if validator.get('email'):
                send_bulk_request_email(
                    ...
                    validator_name=validator.get('name', 'Validator'),
                    request_count=count,
                    updater_name=user.get('name', 'HR Updater')
                )
...
def download_bulk_template():
    ...
    try:
        ...
        # Use get_hr_categories() which excludes employee_raised categories
        hr_categories = get_hr_categories()
...
@hr_bp.route('/updater/delete-record', methods=['POST'])
...
@hr_bp.route('/updater/modify-record', methods=['POST'])
