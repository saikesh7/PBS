...
def pending_requests():
    ...
    try:
        # Get ALL presales categories (active + inactive) for displaying existing requests
        all_presales_category_ids = get_all_presales_category_ids()
        
        if not all_presales_category_ids:
        ...
        if all_presales_category_ids:
            pending_cursor = RequestService.get_pending_requests(user_id, all_presales_category_ids)
...
def processed_requests():
    ...
    try:
        ...
        # ✅ Fetch processed history - ONLY presales department requests
        # Filter by processed_department to show only requests that were processed as presales
        # This ensures that if a category is moved to another department, old records stay with presales
        all_records = []
        ...
        or_conditions = [
            {"processed_department": "presales"}  # New records with processed_department
        ]
        ...
        if all_presales_category_ids:
            or_conditions.append({
                ...
                "category_id": {"$in": all_presales_category_ids}
            })
...
def api_pending_count():
    ...
    try:
        # Get ALL presales categories (active + inactive) for counting existing requests
        all_presales_category_ids = get_all_presales_category_ids()
