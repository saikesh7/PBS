...
from .ld_helpers import (
    ...
    get_ld_categories, get_ld_updater_categories, get_ld_updater_categories_for_display, get_ld_validators, get_month_year_options,
...
def updater_dashboard():
    ...
    try:
        ...
        # Get L&D categories for display - uses hr_categories only to show unique categories without duplicates
        ld_categories = get_ld_updater_categories_for_display()
        ...
        if ld_categories:
            ...
            # Only show records for Direct Award categories created by this updater (like PMO/TA)
            # ✅ FIXED: Only show requests created by this L&D updater
            ...
            # ✅ FIXED: Only show records actually created by this L&D updater
            old_history_cursor = mongo.db.points_request.find({
                "status": {"$in": ["Pending", "Approved", "Rejected"]},
                "created_by_ld_id": ObjectId(user['_id'])  # Only records created by this updater
            }).sort("request_date", -1)
            ...
            for req_data in combined_cursor:
                ...
                if employee and category:
                    ...
                    # Ensure event_date is always a datetime object for quarter calculation
                    event_date = req_data.get("event_date") or req_data.get("request_date")
                    
                    # ✅ Cross-browser compatible quarter calculation
                    ...
                    # ✅ Format event_date safely for display
                    event_date_display = event_date.strftime('%d-%m-%Y') if event_date and isinstance(event_date, datetime) else 'N/A'
        ...
        grades_set = set()
        try:
            # Get grades from updater's own requests (like PMO/TA)
        ...
        # ✅ Get ALL L&D categories (including Employee Raised) for category display only
        all_ld_categories = get_ld_categories()
        ...
        return render_template(
            ...
            all_ld_categories=all_ld_categories,
    ...
    try:
        ...
        for row in validated_rows:
            try:
                ...
                # Ensure category_department is set correctly for L&D
                category_dept = category.get('category_department', 'ld_up')
                if category_dept:
                    category_dept = category_dept.lower()
                else:
                    category_dept = 'ld_up'
                ...
                request_data = {
                    ...
                    "category_department": category_dept,  # Add category_department
                    "points": row['points'],
                ...
                if employee and validator:
                    ...
                    # Ensure category_department is set correctly for realtime event
                    if 'category_department' in category:
                        category['category_department'] = category['category_department'].lower()
                    else:
                        category['category_department'] = 'ld_up'
    ...
    try:
        ...
        for req_data in history_cursor:
            ...
            if employee and category:
                ...
                event_date_display = event_date.strftime('%d-%m-%Y') if event_date and isinstance(event_date, datetime) else 'N/A'
                response_date_display = req_data.get("response_date").strftime('%d-%m-%Y') if req_data.get("response_date") and isinstance(req_data.get("response_date"), datetime) else req_data["request_date"].strftime('%d-%m-%Y')

                history_data.append({
                    'request_id': str(req_data["_id"]),
                    'request_date': req_data["request_date"].strftime('%d-%m-%Y'),
                    'response_date': response_date_display,
                    'event_date': event_date_display,
                    'employee_name': employee.get("name", "Unknown"),
                    'employee_id': employee.get("employee_id", "N/A"),
                    'department': employee.get("department", "N/A"),
        ...
        return jsonify({'success': False, 'error': str(e)}), 500
#updated by Svs Srinivas


@ld_bp.route('/updater/delete-record', methods=['POST'])
...
@ld_bp.route('/updater/modify-record', methods=['POST'])
