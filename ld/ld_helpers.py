...
def get_all_ld_categories():
    """
    Get ALL L&D categories (both active and inactive) from hr_categories collection
    ...
    # Fetch ALL L&D categories (no status filter) - matches pmarch/presales pattern
    ld_categories_new = list(mongo.db.hr_categories.find({
        "category_department": {"$regex": "^(ld.?up|ld.?va|ld)$", "$options": "i"}
    }).sort("name", 1))
...
def get_all_ld_category_ids():
    """Get list of ALL L&D category ObjectIds (active + inactive) - for displaying existing requests"""
    categories = get_all_ld_categories()
    return [cat["_id"] for cat in categories]


def get_ld_category_ids():
    """Get list of ACTIVE L&D category ObjectIds - for new requests"""
    categories = get_ld_categories()
    return [cat["_id"] for cat in categories]
