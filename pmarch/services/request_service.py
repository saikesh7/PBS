"""
Request Service
Handles business logic for PM/Arch request processing
"""
...
class RequestService:
    """Service for handling PM/Arch request operations"""
    ...
    def approve_request(request_id, user_id, notes):
        ...
        try:
            ...
            # Get category to store processed_department
            category = RequestService.get_category_by_id(request_data.get("category_id"))
            processed_department = category.get("category_department", "pmarch") if category else "pmarch"
            ...
            # ✅ PUBLISH REAL-TIME APPROVAL EVENT
            employee = RequestService.get_employee_by_id(request_data.get("user_id"))
            approver = RequestService.get_employee_by_id(user_id)
            ...
            if employee and approver:
                publish_request_approved(
                    request_data=update_data | {
                        ...
                        "category_id": request_data.get("category_id"),
                        "points": request_data.get("points"),
                        "created_by_ta_id": request_data.get("created_by_ta_id")
                    },
    ...
    def reject_request(request_id, user_id, notes):
        ...
        try:
            ...
            # ✅ PUBLISH REAL-TIME REJECTION EVENT
            employee = RequestService.get_employee_by_id(request_data.get("user_id"))
            rejector = RequestService.get_employee_by_id(user_id)
            
            if employee and rejector:
        ...
        try:
            ...
            if file_meta:
                ...
                attachment_info.update({
                    ...
                    "size": size,
                    "download_url": f"/pmarch/get_attachment/{str(request_data['_id'])}",
    ...
    def get_pending_requests(user_id, category_ids):
        """Get all pending requests for a user - filtered by pmarch categories only"""
        try:
            # ✅ Filter by category_ids to show ONLY pmarch requests
            query = {
                "status": "Pending",
    ...
    def get_processed_requests(user_id, category_ids, limit=200):
        """Get processed requests history - filtered by pmarch categories only"""
        try:
            # ✅ Filter by category_ids to show ONLY pmarch requests
            query = {
    ...
    def format_request_for_display(request_data, employee, category):
        ...
        # Format request_date properly - handle both datetime and string
        request_date = request_data.get("request_date")
        if request_date:
            if isinstance(request_date, datetime):
                formatted_date = request_date.strftime('%d-%m-%Y')
                ...
                try:
                    if len(request_date) >= 10:
                        formatted_date = f"{request_date[8:10]}-{request_date[5:7]}-{request_date[0:4]}"
                    else:
