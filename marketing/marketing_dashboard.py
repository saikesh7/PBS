...
# ✅ HELPER FUNCTION: Get ACTIVE marketing category IDs from BOTH collections
def get_marketing_category_ids():
    """Get ACTIVE marketing category IDs from both hr_categories and categories collections"""
    marketing_category_ids = set()
    
    # Get from hr_categories (active only)
...
# ✅ HELPER FUNCTION: Get ALL marketing category IDs (active + inactive) from BOTH collections
def get_all_marketing_category_ids():
    """Get ALL marketing category IDs (active + inactive) from both hr_categories and categories collections
    Used for displaying existing pending/processed requests - shows all regardless of status
    """
    marketing_category_ids = set()
    
    # Get from hr_categories (no status filter - includes inactive)
    hr_marketing_categories = list(mongo.db.hr_categories.find({
        '$or': [
            {'category_department': {'$regex': 'marketing', '$options': 'i'}},
            {'validator': {'$regex': 'marketing', '$options': 'i'}}
...
def dashboard():
    ...
    try:
        ...
        # ✅ Get ALL marketing category IDs (active + inactive) for displaying existing requests
        all_marketing_category_ids = get_all_marketing_category_ids()
        
        # ✅ Get ONLY marketing pending requests count (includes inactive categories)
        query = {
        ...
        if all_marketing_category_ids:
            query['category_id'] = {'$in': all_marketing_category_ids}
        
        pending_count = mongo.db.points_request.count_documents(query)
        
        # Get stats
        
        # Build department filter conditions for processed records
        marketing_dept_conditions = [
            {'processed_department': 'marketing'}
        ]
        if all_marketing_category_ids:
            marketing_dept_conditions.append({
                'processed_department': {'$exists': False},
                'category_id': {'$in': all_marketing_category_ids}
            })
        ...
        # Always try to get stats (even without category_ids, we can filter by processed_department)
        ...
        # ✅ Calculate total points awarded (ALL TIME - not filtered by quarter)
        ...
        # ✅ Get ONLY marketing recent activity (filtered by processed_department)
        all_records = []
        
        # Build department filter conditions
        dept_conditions = [
            {'processed_department': 'marketing'}  # New records with processed_department
        ]
        ...
        if all_marketing_category_ids:
            dept_conditions.append({
                'processed_department': {'$exists': False},
                'category_id': {'$in': list(all_marketing_category_ids)}
            })
...
def pending_requests():
    ...
    try:
        ...
        # ✅ Get ONLY marketing pending requests assigned to this validator (includes inactive)
        query = {
            'assigned_validator_id': ObjectId(user['_id']),
            'status': 'Pending'
        }
...
def process_request(request_id):
    ...
    try:
        ...
        # Get processed_department from category
        processed_department = category.get('category_department', 'marketing') if category else 'marketing'
        ...
        update_data = {
            ...
            'processed_department': processed_department  # Store department at time of processing
        }
...
def processed_requests():
    ...
    try:
        ...
        # Filter by processed_department to show only requests that were processed as marketing
        # This ensures that if a category is moved to another department, old records stay with marketing
        ...
        # ✅ Get pending count (includes inactive categories)
        pending_query = {
            'assigned_validator_id': ObjectId(user['_id']),
            'status': 'Pending'
        }
        ...
        # Add category filter if marketing categories exist
        if all_marketing_category_ids:
            pending_query['category_id'] = {'$in': all_marketing_category_ids}
...
def api_pending_count():
    ...
    try:
        # ✅ Get ALL marketing category IDs (active + inactive) for counting existing requests
        all_marketing_category_ids = get_all_marketing_category_ids()
        
        # Count pending requests assigned to this validator (includes inactive categories)
        query = {
...
@marketing_dashboard_bp.route('/switch_to_employee_view')
def switch_to_employee_view():
    """Switch from marketing view to employee view"""
    ...
    session['original_dashboard'] = 'marketing'
    session['original_view_url'] = url_for('marketing_dashboard.dashboard')
