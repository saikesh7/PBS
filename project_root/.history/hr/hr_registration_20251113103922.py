...
def dashboard():
    ...
    # Fetch DPs - users with role DP
    dps = list(mongo.db.users.find({
        'employee_level': {'$regex': '^dp$', '$options': 'i'}
    }))
    print(f"DEBUG: Found {len(dps)} DPs in database")
    for dp in dps:
        print(f"  - {dp.get('name')} ({dp.get('email')}) - Role: {dp.get('role')}")
    ...
    if 'register_single' in request.form:
        ...
        # Convert manager_id to ObjectId if it's not empty
        if manager_id:
            try:
                if manager_id.strip():
        ...
        # Convert dp_id to ObjectId if it's not empty
        if dp_id:
            try:
                if dp_id.strip():
                    dp_id = ObjectId(dp_id)
                else:
                    dp_id = None
            except Exception as e:
        ...
        try:
            ...
            for row in csv_reader:
                ...
                try:
                    ...
                    # Find manager by name or email
                    manager_id = None
                    manager_name = row.get('manager_name', '').strip()
                    
                    if manager_name:
                        manager = mongo.db.users.find_one({'name': manager_name, 'role': 'Manager'})
                        if manager:
                            manager_id = manager['_id']
                        else:
                            manager = mongo.db.users.find_one({'email': manager_name, 'role': 'Manager'})
                            if manager:
                                manager_id = manager['_id']
                            else:
                                error_messages.append(f"Row {row_num}: Warning - Manager '{manager_name}' not found")

                    # Find DP by name or email
                    dp_id = None
                    ...
                    if dp_name:
                        dp = mongo.db.users.find_one({
                            '$or': [
                                {'name': dp_name, 'role': 'DP'},
                                {'name': dp_name, 'employee_level': {'$regex': '^dp$', '$options': 'i'}}
                            ]
                        })
                        if dp:
                            dp_id = dp['_id']
                            ...
                            dp = mongo.db.users.find_one({
                                '$or': [
                                    {'email': dp_name, 'role': 'DP'},
                                    {'email': dp_name, 'employee_level': {'$regex': '^dp$', '$options': 'i'}}
                                ]
                            })
                            if dp:
                                dp_id = dp['_id']
                            else:
                                error_messages.append(f"Row {row_num}: Warning - DP '{dp_name}' not found")
...
def register_manager():
    ...
    managers = list(mongo.db.users.find({'role': 'Manager'}))
    # Fetch DPs - users with role DP
    dps = list(mongo.db.users.find({'role': 'DP'}).sort('name', 1))
    ...
    if request.method == 'POST':
        if 'search' in request.form:
            ...
            if user:
                ...
                if manager_id:
                    try:
                        update_data['manager_id'] = ObjectId(manager_id)
                    except:
                ...
                if dp_id:
                    try:
                        update_data['dp_id'] = ObjectId(dp_id)
                    except:
                ...
                # Check if DP assignment changed
                old_dp_id = user.get('dp_id')
                new_dp_id = update_data.get('dp_id')
                ...
                # Trigger real-time event for DP dashboard refresh
                if old_dp_id != new_dp_id:
                    try:
                        from services.realtime_events import publish_dp_employee_list_update
                        
                        print(f"DEBUG: DP assignment changed - Old: {old_dp_id}, New: {new_dp_id}")
                        
                        # Notify old DP to refresh
                        if old_dp_id:
                            print(f"DEBUG: Sending refresh to old DP: {old_dp_id}")
                            publish_dp_employee_list_update(str(old_dp_id))
                        
                        # Notify new DP to refresh
                        if new_dp_id:
                            print(f"DEBUG: Sending refresh to new DP: {new_dp_id}")
                            publish_dp_employee_list_update(str(new_dp_id))
                    except Exception as e:
                        print(f"⚠️ Could not send DP refresh notification: {e}")
                        import traceback
                        traceback.print_exc()
...
def manage_fields():
    ...
    return render_template('manage_fields.html', config_data=config_data)
                ...
                try:
                    ...
                    if row.get('manager_name'):
                        manager = mongo.db.users.find_one({
                            '$or': [
                                {'name': row['manager_name'], 'role': 'Manager'},
                                {'email': row['manager_name'], 'role': 'Manager'}
                            ]
                        })
                        if manager:
                    ...
                    if row.get('dp_name'):
                        dp = mongo.db.users.find_one({
                            '$or': [
                                {'name': row['dp_name'], 'role': 'DP'},
                                {'name': row['dp_name'], 'employee_level': {'$regex': '^dp$', '$options': 'i'}},
                                {'email': row['dp_name'], 'role': 'DP'},
                                {'email': row['dp_name'], 'employee_level': {'$regex': '^dp$', '$options': 'i'}}
                            ]
                        })
                        if dp:
