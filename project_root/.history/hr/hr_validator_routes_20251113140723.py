...
def validator_dashboard():
    ...
    try:
        ...
        # Get HR category IDs to filter requests (includes employee_raised)
        hr_category_ids = [cat["_id"] for cat in hr_categories_for_validator] if hr_categories_for_validator else []
...
def handle_bulk_approve(user):
    ...
    approval_notes = request.form.get('approval_notes', 'Bulk approved')
