...
def validator_dashboard():
    ...
    try:
        ...
        analytics_data = calculate_validator_analytics(history_data, validator_requests)
...
def calculate_validator_analytics(history_data, validator_requests):
    """
    Calculate analytics metrics for the validator dashboard
    
    Args:
        history_data: List of historical requests with status
        validator_requests: List of pending requests
    ...
    if not history_data and not validator_requests:
        return analytics
    
    # Count by status
    approved = [h for h in history_data if h.get('status') == 'Approved']
    rejected = [h for h in history_data if h.get('status') == 'Rejected']
    
    analytics['approved_count'] = len(approved)
    analytics['rejected_count'] = len(rejected)
    
    # Calculate approval rate
    total_processed = len(approved) + len(rejected)
    if total_processed > 0:
        analytics['approval_rate'] = int((len(approved) / total_processed) * 100)
    
    # Calculate percentages
    total_all = analytics['total_requests'] + len(validator_requests)
    if total_all > 0:
        analytics['approved_percentage'] = int((len(approved) / total_all) * 100)
        analytics['rejected_percentage'] = int((len(rejected) / total_all) * 100)
        analytics['pending_percentage'] = int((len(validator_requests) / total_all) * 100)
    
    # Calculate average points
    all_points = [h.get('points', 0) for h in history_data if h.get('points')]
    if all_points:
        analytics['avg_points'] = sum(all_points) / len(all_points)
    
    # Requests by grade
    for h in history_data:
        grade = h.get('grade', 'Unknown')
        analytics['requests_by_grade'][grade] = analytics['requests_by_grade'].get(grade, 0) + 1
    ...
    for h in history_data:
        ...
        if category not in analytics['top_categories']:
            analytics['top_categories'][category] = {
                'count': 0,
                'total_points': 0
            }
        
        analytics['top_categories'][category]['count'] += 1
        analytics['top_categories'][category]['total_points'] += points
    
    # Sort top categories by count (descending) and limit to top 5
    sorted_categories = sorted(
        analytics['top_categories'].items(),
        key=lambda x: x[1]['count'],
        reverse=True
    )[:5]
    analytics['top_categories'] = dict(sorted_categories)
