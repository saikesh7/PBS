...
def handle_single_assignment(user, current_tab='single-request'):
    ...
    if points == 0:
        flash(f'Grade "{employee_grade}" is not eligible for category "{category.get("name")}". Please select a different category or employee.', 'danger')
        return redirect(url_for('ta.updater_dashboard', tab=current_tab))
    ...
    # Send email notification to validator (non-blocking)
    try:
        from ta.ta_email_service import send_new_request_email
        ...
        if validator.get('email'):
            ...
            print(f"✅ TA Email notification sent to validator {validator.get('name')} for single request")
    except Exception as e:
