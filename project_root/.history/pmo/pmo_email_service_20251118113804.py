...
def send_new_request_email(validator_email, validator_name, employee_name, category_name, points, event_date, notes, utilization_value=None):
    ...
    <div class="content">
        ...
        <div class="info-box">
            ...
            <div class="info-row">
                <span class="label">{points_or_util_label}</span>
                <span class="value">{points_or_util_value}</span>
            </div>
            <div class="info-row">
...
def send_approval_email_to_updater(updater_email, updater_name, employee_name, category_name, points, validator_name, utilization_value=None):
...
def send_approval_email_to_employee(employee_email, employee_name, category_name, points, event_date, utilization_value=None):
    """Send approval notification to employee"""
    
    # Determine if this is utilization or points
    if utilization_value is not None:
        subject = f"Utilization Approved - {int(utilization_value * 100)}% Recorded"
        points_or_util_label = "Utilization:"
        points_or_util_value = f"{int(utilization_value * 100)}%"
        message = "Great news! Your utilization has been approved and recorded."
        credit_message = "This utilization has been recorded in your account."
    else:
        subject = f"Reward Approved - {points} Points Awarded"
        points_or_util_label = "Points Awarded:"
        points_or_util_value = f"{points} Points"
        message = "Great news! You have been awarded points for your outstanding performance."
        credit_message = "These points have been awarded to you for your excellent work."
    ...
    <div class="content">
        ...
        <div class="info-box" style="border-left-color: #28a745;">
            ...
            <div class="info-row">
                <span class="label">{points_or_util_label}</span>
                <span class="value success">{points_or_util_value}</span>
            </div>
            <div class="info-row">
                <span class="label">Event Date:</span>
                <span class="value">{event_date}</span>
