...
def raise_request():
    ...
    for req in pending_reqs:
        # Detect if this is a direct award (created by HR/PMO/L&D/TA or tagged source)
        is_direct_award = False
        creator_user = None
        for creator_field in ('created_by_hr_id', 'created_by_pmo_id', 'created_by_ld_id', 'created_by_ta_id'):
            if req.get(creator_field):
                is_direct_award = True
                try:
                    creator_user = mongo.db.users.find_one({'_id': req.get(creator_field)})
                except:
                    creator_user = None
                break

        if not is_direct_award:
            # also check explicit source flag if present
            if req.get('source') and str(req.get('source')).lower() == 'direct_award':
                is_direct_award = True
        ...
        validator = None
        try:
            if req.get('assigned_validator_id'):
                validator = mongo.db.users.find_one({'_id': req.get('assigned_validator_id')})
        except:
            validator = None

        # Prefer category name from category doc if available, else fallback to stored field or a direct-award label
        if category:
            category_name = category.get('name', 'Unknown')
            category_department = category.get('category_department', 'Unknown')
        else:
            category_name = req.get('category_name') or ('Direct Award' if is_direct_award else 'Unknown')
            category_department = req.get('category_department') or ('Direct Award' if is_direct_award else 'Unknown')

        # If no assigned validator but this is a direct award, show the creator as the validator/awarder
        assigned_validator_name = None
        assigned_validator_emp_id = None
        if validator:
            assigned_validator_name = validator.get('name')
            assigned_validator_emp_id = validator.get('employee_id')
        elif is_direct_award and creator_user:
            assigned_validator_name = creator_user.get('name')
            assigned_validator_emp_id = creator_user.get('employee_id')
        ...
        pending_requests.append({
            ...
            'assigned_validator_name': assigned_validator_name,
            'assigned_validator_emp_id': assigned_validator_emp_id,
