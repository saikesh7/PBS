...
# ✅ BACKGROUND TASK: Send all notifications asynchronously
def send_notifications_background(request_doc, employee, validator_user, category):
    """
    Send validator and marketing notifications in background thread.
    This prevents blocking the user's request while emails are being sent.
    """
    try:
        # Send to assigned validator
        send_validator_notification(request_doc, employee, validator_user, category)
    except Exception as e:
        pass  # Silently fail, don't block request
    
    try:
        # Send to all marketing validators
        ...
        for marketing_validator in marketing_validators:
            try:
                send_validator_notification(request_doc, employee, marketing_validator, category)
            except Exception as e:
                pass  # Silently fail, continue to next
    except Exception as e:
        pass  # Silently fail


# ✅ BACKGROUND TASK: Publish realtime events asynchronously
def publish_realtime_background(request_doc, employee, validator_user, category):
    """
    Publish realtime events in background thread.
    This prevents blocking while socket events are emitted.
    """
    try:
        from services.realtime_events import publish_request_raised
        publish_request_raised(request_doc, employee, validator_user, category)
    except Exception as e:
        pass  # Silently fail, don't block request
...
def submit_request():
    ...
    try:
        ...
        # ✅ OPTIMIZATION: Fetch validator once and reuse
        validator_user = mongo.db.users.find_one({"_id": ObjectId(validator_id)})
        
        # ✅ OPTIMIZATION: Start background threads for emails and realtime events
        # This ensures user sees success immediately while notifications happen in background
        if validator_user:
            # Thread 1: Publish realtime events (fast but async)
            event_thread = threading.Thread(
                target=publish_realtime_background,
                args=(request_doc, user, validator_user, category),
                daemon=True,
                name='RealtimeEventThread'
            )
            event_thread.start()
            
            # Thread 2: Send all email notifications (blocking SMTP, so definitely needs background)
            email_thread = threading.Thread(
                target=send_notifications_background,
                args=(request_doc, user, validator_user, category),
                daemon=True,
                name='EmailNotificationThread'
            )
            email_thread.start()
        
        # ✅ USER SEES SUCCESS IMMEDIATELY - emails/events sent in background
        if is_pm_category:
            manager_name = validator_user.get('name', 'your manager') if validator_user else 'your manager'
