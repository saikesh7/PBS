        ...
        try:
            ...
            for req in points_requests:
                try:
                    ...
                    # If the request is Approved and not already present in points_map,
                    # we need to count its points in totals too (this covers approved
                    # requests that haven't been migrated into the points collection).
                    if req.get('status') == 'Approved':
                        # Determine category, utilization and bonus flags first
                        ...
                        # If this request carries numeric points, add to totals
                        points_value = req.get('points', 0)
                        try:
                            if isinstance(points_value, (int, float)):
                                # Only aggregate bonus points here. Regular approved
                                # request points remain counted from `points` collection
                                # (where regular approved entries should live).
                                if is_bonus:
                                    total_bonus_points += points_value
                        except Exception:
                            # Silently ignore malformed numeric values
                            pass

                        # Continue to display this approved request like a normal entry
