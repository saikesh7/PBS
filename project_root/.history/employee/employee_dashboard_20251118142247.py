...
def error_print(message, error=None):
    print(f"ERROR - EMPLOYEE: {message}", file=sys.stderr)
    if error:
...
def dashboard():
    ...
    print("="*80, flush=True)
    print("🔍 UTILIZATION FETCH STARTING", flush=True)
    print("="*80, flush=True)
    try:
        print(f"🔍 UTILIZATION DEBUG: Starting fetch for user_id: {user_id}", flush=True)
        ...
        if utilization_category:
            print(f"✅ UTILIZATION DEBUG: Found category in hr_categories: {utilization_category.get('name')} with ID: {utilization_category['_id']}", flush=True)
        else:
            ...
            if utilization_category:
                print(f"✅ UTILIZATION DEBUG: Found category in categories: {utilization_category.get('name')} with ID: {utilization_category['_id']}", flush=True)
        ...
        if utilization_category:
            ...
            print(f"🔍 UTILIZATION DEBUG: Query: {query}", flush=True)
            ...
            for util_record in utilization_cursor:
                print(f"📊 UTILIZATION DEBUG: Found record: {util_record.get('_id')}", flush=True)
                utilization_value = util_record.get('utilization_value', 0)
                event_date = util_record.get('event_date')
                print(f"   - utilization_value: {utilization_value}", flush=True)
                print(f"   - event_date: {event_date}, type: {type(event_date)}", flush=True)
                ...
                if event_date and isinstance(event_date, datetime):
                    ...
                    print(f"   ✅ Added record: {month_year_str} - {utilization_value * 100}%", flush=True)
                    
                    total_utilization += utilization_value
                    count += 1
                else:
                    print(f"   ❌ Skipped - invalid event_date", flush=True)
            ...
            print(f"✅ UTILIZATION DEBUG: Found {count} records. Average: {average_utilization}%", flush=True)
            print(f"✅ UTILIZATION DEBUG: Total records in list: {len(utilization_records)}", flush=True)
        else:
            print(f"❌ UTILIZATION DEBUG: No utilization category found in categories or hr_categories", flush=True)
    
    except Exception as e:
        print(f"❌ UTILIZATION DEBUG ERROR: {str(e)}", flush=True)
        import traceback
    ...
    print("="*80, flush=True)
    print(f"🎨 RENDER DEBUG: Passing to template:", flush=True)
    print(f"   - utilization_records count: {len(utilization_records)}", flush=True)
    print(f"   - utilization_records: {utilization_records}", flush=True)
    print(f"   - average_utilization: {average_utilization}", flush=True)
    print(f"   - utilization_months count: {len(utilization_months)}", flush=True)
    print(f"   - utilization_months: {utilization_months}", flush=True)
    if utilization_records:
        print(f"   - First record: {utilization_records[0]}", flush=True)
    print("="*80, flush=True)
...
def get_request_attachment(request_id):
    ...
    try:
        ...
        print(f"📥 EMPLOYEE REQUEST HISTORY DEBUG: Downloading attachment for request: {request_id}", flush=True)
        ...
        if not req:
            print(f"❌ EMPLOYEE REQUEST HISTORY DEBUG: Request not found or doesn't belong to user", flush=True)
        ...
        print(f"📥 EMPLOYEE REQUEST HISTORY DEBUG: Looking for file ID: {attachment_id}", flush=True)
        
        if not fs.exists(attachment_id):
            print(f"❌ EMPLOYEE REQUEST HISTORY DEBUG: File not in GridFS", flush=True)
        ...
        print(f"✅ EMPLOYEE REQUEST HISTORY DEBUG: File found, retrieving...", flush=True)
        grid_out = fs.get(attachment_id)
        file_data = grid_out.read()
        print(f"✅ EMPLOYEE REQUEST HISTORY DEBUG: Retrieved {len(file_data)} bytes", flush=True)
        ...
        print(f"✅ EMPLOYEE REQUEST HISTORY DEBUG: Sending: {original_filename}", flush=True)
        ...
        print(f"❌ EMPLOYEE REQUEST HISTORY DEBUG: Error: {str(e)}", flush=True)
        import traceback
        print(traceback.format_exc(), flush=True)
