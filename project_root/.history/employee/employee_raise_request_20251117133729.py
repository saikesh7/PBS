...
def submit_request():
    """Handle form submission for raise request - OPTIMIZED & FIXED"""
    user_id = session.get('user_id')
    if not user_id:
    ...
    try:
        # Get form data with flexible field names
        ...
        # Determine and validate validator
        if is_pm_category:
            # PM categories automatically use assigned manager
            ...
            # Verify manager exists and has PM access
            ...
            # Non-PM categories require explicit validator selection
            if not validator_id:
            ...
            try:
                ...
                flash('Invalid validator ID format. Please try again.', 'danger')
                return redirect(url_for('employee_raise_request.raise_request'))
            ...
            # Verify validator has access to this department
        ...
        # Send real-time notification (fail silently if error)
        try:
        ...
        # Send email notification (fail silently if error)
        try:
            validator_user = mongo.db.users.find_one({"_id": ObjectId(validator_id)})
            if validator_user:
        ...
        flash(f'An unexpected error occurred while submitting your request. Please try again or contact support.', 'danger')
        return redirect(url_for('employee_raise_request.raise_request'))


@employee_raise_request_bp.route('/api/get-validators-by-category/<category_id>')
def get_validators_by_category(category_id):
    """API to get validators for a specific category - OPTIMIZED"""
    user_id = session.get('user_id')
    if not user_id:
    ...
    try:
        ...
        if is_pm_category:
            # PM category - return manager info
            user = mongo.db.users.find_one({'_id': ObjectId(user_id)})
        ...
        # Non-PM category - find validators with dashboard access
