                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #d1ecf1; border-radius: 6px; padding: 20px; border-left: 4px solid #0dcaf0;">
                                <tr>
                                    <td>
                                        ...
                                        <p style="color: #333; font-size: 14px; line-height: 1.6; margin: 0;">Please log in to the Prowess Points System to review this request and take appropriate action. You can approve, reject, or request additional information from the employee.</p>
                                    </td>
                                </tr>
                            </table>
                        </td>
...
⏰ ACTION REQUIRED
───────────────────────────────────────────────────────────
Please log in to the Prowess Points System to review this request 
and take appropriate action. You can approve, reject, or request 
additional information from the employee.

Visit: http://your-application-url.com/validator/dashboard
...
def submit_request():
    ...
    try:
        ...
        if is_pm_category:
            ...
            if not manager:
                flash('Assigned manager not found. Please contact HR.', 'danger')
                return redirect(url_for('employee_raise_request.raise_request'))
            
            if 'pm' not in manager.get('dashboard_access', []):
                flash('Your assigned manager does not have PM dashboard access. Please contact HR.', 'danger')
                return redirect(url_for('employee_raise_request.raise_request'))
        else:
            if not validator_id:
                flash('Please select a validator', 'danger')
                return redirect(url_for('employee_raise_request.raise_request'))
        ...
        if attachment and attachment.filename:
            try:
                ...
                flash('Error uploading attachment', 'warning')
        ...
        if is_pm_category:
            flash('Request submitted successfully to your assigned manager!', 'success')
        else:
...
def get_validators_by_category(category_id):
    """API to get validators for a specific category based on category_department"""
    user_id = session.get('user_id')
    if not user_id:
    ...
    try:
        ...
        category = mongo.db.hr_categories.find_one({'_id': category_obj_id})
        ...
        if is_pm_category:
            ...
            if not manager or 'pm' not in manager.get('dashboard_access', []):
                return jsonify({
        ...
        if not validators:
            return jsonify({
                'error': f'No validators found with {category_department} dashboard access. Please contact HR.',
                'success': False,
