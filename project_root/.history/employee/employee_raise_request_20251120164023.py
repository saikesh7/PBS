...
# ✅ BACKGROUND TASK: Send emails asynchronously
def send_emails_background(request_doc, user, validator_user, category):
    """Send validator and marketing notifications in background thread (non-blocking)"""
    try:
        # Send to assigned validator
        send_validator_notification(request_doc, user, validator_user, category)
    except Exception as e:
        pass  # Log silently, don't block request
...
def publish_events_background(request_doc, user, validator_user, category):
    """Publish realtime events in background thread (non-blocking)"""
    try:
...
def submit_request():
    ...
    try:
        ...
        # ✅ OPTIMIZATION: Send emails and events in BACKGROUND THREADS (non-blocking)
        if validator_user:
            # Start background thread for realtime events (very fast, but still async)
            event_thread = threading.Thread(
                target=publish_events_background,
                args=(request_doc, user, validator_user, category),
                daemon=True
            )
            event_thread.start()
            
            # Start background thread for email notifications (blocking SMTP)
        ...
        # ✅ USER RESPONSE IS IMMEDIATE (emails sent in background)
        if is_pm_category:
