        ...
        for point in points_entries:
            try:
                ...
                if is_utilization:
                    ...
                    print(f"This is a utilization entry, checking for utilization_value", flush=True)
                    ...
                    if point_request_id:
                        try:
                            ...
                            if original_request and 'utilization_value' in original_request:
                                ...
                                print(f"Found utilization_value: {utilization_value} -> {utilization_percentage}%", flush=True)
                        except Exception as e:
                            print(f"⚠️ Error fetching utilization from request: {str(e)}", flush=True)
                    ...
                    if not display_value and 'utilization_value' in point:
                        ...
                        utilization_percentage = round(utilization_value * 100, 2)
                        display_value = f"{utilization_percentage}%"
                        print(f"Found utilization_value in point: {utilization_value} -> {utilization_percentage}%", flush=True)
                    
                    # Final fallback
                    if not display_value:
                        display_value = "N/A"
                        print(f"No utilization_value found, using N/A", flush=True)
                else:
                    # Regular points
                ...
                print(f"✅ Successfully processed point {point_id} with display_value: {display_value}", flush=True)
