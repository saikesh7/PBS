...
def raise_request():
    ...
    pending_requests = []
    # Exclude direct awards from the employee's Pending list. Direct awards
    # are created by HR/PMO/L&D and use source='direct_award'. We only want
    # to show employee-raised pending requests here.
    pending_reqs = mongo.db.points_request.find({
        'user_id': ObjectId(user_id),
        'status': 'Pending',
        'source': {'$ne': 'direct_award'}
    }).sort('request_date', -1)
