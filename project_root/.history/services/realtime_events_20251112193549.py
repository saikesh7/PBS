    ...
    try:
        ...
        if dashboard_type.startswith('hr') or dashboard_type == 'hr_up':
            ...
            target_role = 'pm_arch'
             ...
             # Fallback to validator_access if category_department doesn't match
            if 'hr_va' in validator_access:
                event_type = 'new_request'
                target_role = 'hr_validator'
            elif 'ta_va' in validator_access:
                event_type = 'new_request'
                target_role = 'ta_validator'
            elif 'pmo_va' in validator_access:
                event_type = 'new_request'
                target_role = 'pmo_validator'
        else:
            # Default
