...
def publish_request_raised(request_data: Dict, employee_data: Dict, validator_data: Dict, category_data: Dict):
    ...
    try:
        ...
        source = "TA Updater" if request_data.get('created_by_ta_id') else ("PMO Updater" if request_data.get('created_by_pmo_id') else ("HR Updater" if request_data.get('created_by_hr_id') else "Employee"))
...
def publish_request_approved(request_data: Dict, employee_data: Dict, approver_data: Dict, points_award_data: Optional[Dict] = None):
    ...
    try:
        ...
        # Line 227-237: Notifies HR Updater when request is approved
        ...
        if 'created_by_hr_id' in request_data and request_data.get('created_by_hr_id'):
            ...
            if hr_updater_id != employee_id:
                ...
                print(f"✅ Real-Time: Approval notification sent to HR Updater (ID: {hr_updater_id}) in room: hr_updater_{hr_updater_id}")

        
        
        
        # ✅ 4. Notify L&D Updater who raised the request (if different from employee)
        ...
        if employee_data.get('dp_id'):
            ...
            print(f"✅ Real-Time: DP notification sent for employee {employee_data.get('name')} to DP {dp_id}")
...
def publish_request_rejected(request_data: Dict, employee_data: Dict, rejector_data: Dict):
    ...
    try:
        ...
        # Line 318-328: Notifies HR Updater when request is rejected
        if 'created_by_hr_id' in request_data and request_data.get('created_by_hr_id'):
            hr_updater_id = str(request_data.get('created_by_hr_id'))
            employee_id = str(employee_data.get('_id'))
            if hr_updater_id != employee_id:
                ...
                print(f"✅ Real-Time: Rejection notification sent to HR Updater (ID: {hr_updater_id}) in room: hr_updater_{hr_updater_id}")
        ...
        if employee_data.get('dp_id'):
            ...
            print(f"✅ Real-Time: DP notification sent for rejected request for employee {employee_data.get('name')} to DP {dp_id}")
...
def publish_points_awarded_direct(points_award_data: Dict, employee_data: Dict, awarder_data: Dict, category_data: Dict):
    ...
    try:
        ...
        if employee_data.get('dp_id'):
            ...
            print(f"✅ Real-Time: DP notification sent for direct award to employee {employee_data.get('name')} to DP {dp_id}")
...
def publish_dp_employee_list_update(dp_id: str):
    ...
    redis_service, _ = get_services()
    if not redis_service:
        print("⚠️ Redis service not available for publish_dp_employee_list_update")
        return False
    ...
    try:
        ...
        print(f"✅ Real-Time: DP employee list update notification sent to DP {dp_id}")
        return True
    
    except Exception as e:
        print(f"❌ Error in publish_dp_employee_list_update: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        return False
...
def publish_dp_points_update(employee_id: str, dp_id: str, points_data: Dict):
    ...
    redis_service, _ = get_services()
    if not redis_service:
        print("⚠️ Redis service not available for publish_dp_points_update")
        return False
    ...
    try:
        ...
        print(f"✅ Real-Time: DP points update notification sent to DP {dp_id} for employee {employee_id}")
        return True
    
    except Exception as e:
        print(f"❌ Error in publish_dp_points_update: {e}", file=sys.stderr)
        import traceback
        traceback.print_exc()
        return False