...
def send_new_request_email(validator_email, validator_name, employee_name, category_name, points, event_date, notes, utilization_value=None):
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f59e0b; border-radius: 12px 12px 0 0;">
        <tr>
            <td style="padding: 35px 30px; text-align: center;">
                ...
                <h1 style="margin: 0; font-size: 24px; font-weight: 700; color: #ffffff; letter-spacing: -0.5px;">New PMO Reward Request</h1>
    ...
    <div style="padding: 35px; background: #ffffff;">
        ...
        <p style="font-size: 15px; color: #4b5563; line-height: 1.6; margin-bottom: 25px;">A new PMO reward request has been submitted and requires your validation.</p>
        ...
        <div style="background: #f9fafb; border-left: 4px solid #3b82f6; padding: 25px; border-radius: 6px; margin: 25px 0;">
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                    <td style="padding: 10px 0; color: #6b7280; font-weight: 600; font-size: 14px;">{points_or_util_label}</td>
                    <td style="padding: 10px 0; color: #1f2937; font-size: 15px; font-weight: 500;">{points_or_util_value}</td>
                </tr>
                <tr>
...
def send_bulk_request_email(validator_email, validator_name, request_count, updater_name):
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #f59e0b; border-radius: 12px 12px 0 0;">
        <tr>
            <td style="padding: 35px 30px; text-align: center;">
                ...
                <h1 style="margin: 0; font-size: 24px; font-weight: 700; color: #ffffff; letter-spacing: -0.5px;">PMO Bulk Upload Notification</h1>
    ...
    <div style="padding: 35px; background: #ffffff;">
        <p style="font-size: 16px; color: #1f2937; margin-bottom: 8px;">Hello <strong>{validator_name}</strong>,</p>
        <p style="font-size: 15px; color: #4b5563; line-height: 1.6; margin-bottom: 25px;"><strong>{updater_name}</strong> has submitted multiple PMO reward requests that require your validation.</p>
...
def send_approval_email_to_updater(updater_email, updater_name, employee_name, category_name, points, validator_name, event_date=None, utilization_value=None, submission_notes="", validator_notes=""):
    ...
    # Truncate notes to 100 characters
    validator_notes = truncate_notes(validator_notes)
    ...
    <div class="content">
        ...
        <p>Your PMO reward request has been approved by the validator.</p>
        ...
        <p style="margin-top: 20px;">{credit_message}</p>
...
def send_rejection_email_to_updater(updater_email, updater_name, employee_name, category_name, validator_name, rejection_notes, points=0, submission_notes=""):
    ...
    # Truncate notes to 100 characters
    rejection_notes = truncate_notes(rejection_notes)
    submission_notes = truncate_notes(submission_notes)
    ...
    <div class="header" style="background-color: #ef4444;">
        <h1>PMO Points Rejected</h1>
        <p style="margin: 5px 0 0 0; font-size: 14px; opacity: 0.95;">Request has been rejected by validator</p>
    </div>
    <div class="content">
        <p>Hello <strong>{updater_name}</strong>,</p>
        <p>Your PMO reward request has been rejected by the validator. Please review the details below.</p>
