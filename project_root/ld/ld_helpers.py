...
def get_ld_categories():
    """Get all L&D categories from hr_categories ONLY (new system)"""
    mongo = get_mongo()
    
    # Fetch ONLY from hr_categories - Direct Award and Employee Raised types (for validators)
...
def get_ld_updater_categories():
    """Get L&D categories for updaters from hr_categories ONLY - Only Direct Award type (excludes Employee raised)"""
    mongo = get_mongo()
    
    # Fetch ONLY from hr_categories - Updaters should only see Direct Award categories, not Employee raised
    ld_categories = list(mongo.db.hr_categories.find({
        "category_department": {"$regex": "^(ld.?up|ld.?va)$", "$options": "i"},
        "category_status": {"$regex": "^active$", "$options": "i"},
...
SELECTABLE_EMPLOYEE_FILTER_FOR_LD = {
    # Include all users who have an employee_id (excludes system accounts without employee_id)
