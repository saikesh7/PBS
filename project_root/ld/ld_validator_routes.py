...
def validator_dashboard():
    ...
    try:
        ...
        if not ld_categories:
            ...
            # Include both assigned requests and employee-raised requests
            # For employee-raised requests, show to ALL validators (not filtered by assigned_validator_id)
            ...
            pending_query = {
                ...
                "$or": [
                    ...
                    },  # OLD: Pending for this validator (old field name)
                    {
                        "category_id": {"$in": category_ids},
                        "source": "employee"
                    }  # Employee-raised requests in LD categories
                ]
            }
...
def handle_single_action(user):
    ...
    try:
        ...
        if action == 'approve':
            ...
            if employee_data and category_data and updater_data:
...
def handle_bulk_approve(user):
    ...
    if approved_requests_data:
        ...
        from ld.ld_email_service import send_bulk_approval_emails
...
def check_new_requests():
    ...
    try:
        ...
        # Get total pending count - include both assigned and employee-raised requests
        # Support both old (pending_validator_id) and new (assigned_validator_id) field names
