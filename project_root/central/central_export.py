...
def export_excel():
    ...
    try:
        ...
        tomorrow = current_dt.replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1)
        if end_dt >= tomorrow:
        ...
        try:
            ...
            create_utilization_worksheet(worksheet_util, employee_data, start_date_str, end_date_str,
                                        title_format, header_format, data_format, number_format)
...
def create_summary_worksheet(worksheet, employee_data, all_categories, start_date_str, end_date_str,
    ...
    for emp_data in employee_data:
        ...
        worksheet.write(row, col, emp_data.get('utilization', 0), number_format); col += 1
...
def create_utilization_worksheet(worksheet, employee_data, start_date_str, end_date_str,
                                 title_format, header_format, data_format, number_format):
    ...
    util_headers = ['Name', 'Email'] + all_months
    ...
    for emp_data in employee_data:
        if emp_data.get('monthly_utilization'):
            col = 0
            worksheet.write(row, col, emp_data.get('name', ''), data_format); col += 1
            worksheet.write(row, col, emp_data.get('email', ''), data_format); col += 1
            ...
            for month_header in all_months:
                ...
                worksheet.write(row, col, util_value, number_format)
                col += 1
            row += 1
    ...
    worksheet.set_column('B:B', 30)
    if all_months:
        worksheet.set_column(2, 2 + len(all_months) - 1, 15)
...
def process_employee_for_export(user, all_categories, start_dt, end_dt):
    ...
    try:
        ...
        points_query = {"user_id": user_id, "status": "Approved", "request_date": {"$gte": start_dt, "$lte": end_dt}}
        
        # Get all approved points for this user in the date range
        points_results = list(mongo.db.points_request.find(points_query).sort('request_date', 1))
        ...
        for point_record in points_results:
            ...
            point_breakdown.append({
                ...
                'points': points_val,
                'request_date': point_record.get('request_date', '').strftime('%Y-%m-%d') if point_record.get('request_date') else '',
                'is_bonus': is_bonus
            })
