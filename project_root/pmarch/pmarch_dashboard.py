...
def dashboard():
    ...
    try:
        ...
        history_cursor = mongo.db.points_request.find(history_query).sort("processed_date", -1).limit(100)
        ...
        # Calculate quarterly statistics (including old data - no category filter)
        quarterly_stats = {}
        for grade_key in ALL_GRADES:
            ...
            # Calculate employees with pmarch points (including old data - no category filter)
            employees_with_pmarch = mongo.db.points_request.distinct("user_id", {
                "user_id": {"$in": grade_employee_ids},
                "request_date": {"$gte": quarter_start},
                "status": "Approved",
                "processed_by": ObjectId(user_id)
            })
            
            # Sum of approved pmarch points (including old data - no category filter)
