...
def dashboard():
    ...
    try:
        ...
        # ============================================
        # ✅ SIMPLE APPROACH: Calculate totals from points collection (like HR Analytics)
        # ============================================
        try:
            ...
            points_entries = list(mongo.db.points.find({"user_id": ObjectId(user_id)}))
            ...
            for point in points_entries:
                ...
                is_utilization = category_id in utilization_category_ids if category_id else False
        ...
        # ✅ Calculate current quarter and year totals from points collection
        now_utc = datetime.utcnow()
        ...
        # ✅ Calculate quarterly and yearly totals from points collection
        try:
            for point in points_entries:
                category_id = point.get('category_id')
                points_value = point.get('points', 0)
                
                # Get the effective date (award_date or event_date)
                ...
                # Only count regular points (not bonus, not utilization)
                if not is_utilization and not is_bonus:
