...
def get_all_approved_points_for_leaderboard(filters=None):
    ...
    # Parse date filter from quarter selection
    date_range = None
    selected_quarter_filter = filters.get('quarter', 'all')
    
    if selected_quarter_filter != 'all' and '-' in selected_quarter_filter:
        try:
    ...
    points_entries = mongo.db.points.find(p_query)
...
def leaderboard():
    ...
    try:
        ...
        # Get initial leaderboard data (no filters)
        initial_leaderboard_data = get_leaderboard_data_with_rank(str(user_id))
