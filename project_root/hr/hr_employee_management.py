...
def employee_list():
    ...
    # Dynamically get departments from actual employee data
    departments = mongo.db.users.distinct('department', {'role': {'$in': ['Employee', 'DP']}, 'department': {'$exists': True, '$ne': None, '$ne': ''}})
    departments = sorted([dept for dept in departments if dept])  # Remove empty values and sort
    
    # Dynamically get grades from actual employee data
    grades = mongo.db.users.distinct('grade', {'role': {'$in': ['Employee', 'DP']}, 'grade': {'$exists': True, '$ne': None, '$ne': ''}})
    grades = sorted([grade for grade in grades if grade])  # Remove empty values and sort
    
    # Dynamically get locations from actual employee data (check both fields)
    locations_us_non_us = mongo.db.users.distinct('us_non_us', {'role': {'$in': ['Employee', 'DP']}, 'us_non_us': {'$exists': True, '$ne': None, '$ne': ''}})
    locations_location = mongo.db.users.distinct('location', {'role': {'$in': ['Employee', 'DP']}, 'location': {'$exists': True, '$ne': None, '$ne': ''}})
    locations = sorted(list(set([loc for loc in locations_us_non_us + locations_location if loc])))  # Merge and remove duplicates
