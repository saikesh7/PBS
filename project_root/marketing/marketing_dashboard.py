...
def dashboard():
    ...
    try:
        ...
        # Get marketing category IDs for statistics only
        marketing_category_ids = set()
        marketing_categories = list(mongo.db.hr_categories.find({
            '$or': [
                {'category_department': {'$regex': 'marketing', '$options': 'i'}},
                {'validator': {'$regex': 'marketing', '$options': 'i'}}
            ]
        }))
        if not marketing_categories:
            marketing_categories = list(mongo.db.categories.find({
                'validator': {'$regex': 'marketing', '$options': 'i'}
            }))
        ...
        if marketing_category_ids:
            query['category_id'] = {'$in': list(marketing_category_ids)}
        ...
        stats_filter = {}
        if marketing_category_ids:
            stats_filter = {'category_id': {'$in': list(marketing_category_ids)}}
...
def raise_request():
    ...
    try:
        ...
        try:
            from services.realtime_events import publish_employee_request_raised
            publish_employee_request_raised(
